...
def quantumvolume_qc_benchmark(nb_qubits=2):
    ...
    data_map_func = [1,0]
    qc_qv = ZZFeatureMap(feature_dimension=nb_qubits, reps=2, entanglement="linear",data_map_func=data_map_func).decompose()
    qc_qv.measure_all()
...
quantumvolume_qc_benchmark(2)
for q in range(2,4):
    quantumvolume_qc_benchmark(q)
    dict = {"data_q": data_q, "data_t": data_t}
    df = pd.DataFrame(dict)
    df.to_csv("qsvm_qc-single-data.csv")
sys.stdout = LogFile("memory_profile_log-qsvm_qc-single", reportIncrementFlag=True)
